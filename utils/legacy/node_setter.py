...
from . import file, node_parser, versioning
...
def open_tex(cobj):
    ...
    if images.find(tex_name) != -1:
        ...
        if not file.exist_path(old_tex_path) or not utils.compare_size_same(old_tex_path, tex_path):
            old_name = tex_name
            tex_name = utils.ensure_unique_name(tex_name, images.keys())
...
def new_element(obj, cobj, attr_name, ops: None|bpy.types.Operator=None):
    ...
    # bpy.types.ColorRamp, elements, bpy.types.ColorRampElement(s)
    if attr_name == "elements":
        new(cobj["position"])
    # bpy.types.CurveMap, points, bpy.types.CurveMapPoints
    elif attr_name == "points":
        new(cobj["location"][0], cobj["location"][1])
    # bpy.types.NodeEnumDefinition, enum_items, bpy.types.NodeEnumItem(s)
...
def try_setattr(obj, cobj, attr, cvalue, ops: None|bpy.types.Operator=None, invoker: str|int|None=None):
    ...
    # Setting Read-only attr may cause this
        ...
        if "read-only" not in e.args[0]:
            report(ops, {'WARNING'}, "rpt_warning_setter_soft_error_universal")
    ...
    # Set float to Vector may cause this (blender record 0.0x4 as 0.0?)
        ...
        report(ops, {'ERROR'}, "rpt_error_setter_universal")
        ...
        if length == clength:
            ...
            for i in range(clength):
                ...
                try:
                    ...
                    report(ops, {'ERROR'}, "rpt_error_setter_universal" + " (Index Error)")
                except Exception as e:
                    report(ops, {'ERROR'}, "rpt_error_setter_universal")
...
def set_nodes(node_tree, nodes, cnodes, cnode_trees, node_offset=Vector((0.0, 0.0)), set_tree_io=False, ops: None|bpy.types.Operator=None):
    ...
    for cnode in cnodes.values():
        ...
        if bl_idname in constants.NODE_GROUP_IDNAMES:
            if cnode.get("HN_nt_name", None) is not None:
                ...
                # nesting a node group inside of itself is not allowed
                    ...
                    ops.report({'WARNING'}, "rpt_warning_setter_ng_nesting".format(tree_name=node_tree.name))
                    continue
        # set node's image
...
def apply_preset(context: bpy.types.Context, preset_name, cpreset: dict, dst_tree, apply_offset=False, new_tree=False, ops: None|bpy.types.Operator=None):
    ...
    for cname, cnode_tree in cnode_trees.items():
        ...
        cname = utils.ensure_unique_name(cname, node_groups.keys())
    ...
    # if is base node tree, skip setting io for trees except geo tree
    elif edit_tree.bl_idname != "GeometryNodeTree":
        set_tree_io = False
        link_group_io = False
        ...
        # if tree io is not capatible and has group io node, let user to choose whether to reset tree io or not
            ...
            set_tree_io = True # always true, only for versioning
