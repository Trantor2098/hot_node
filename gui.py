...
class HOTNODE_MT_specials(Menu):
    ...
    def draw(self, context):
        ...
        # Add-on Settings
        layout.separator()
        layout.prop(addon_prefs, "in_one_menu", text=i18n.msg["In One Menu"])
        layout.prop(addon_prefs, "extra_confirm", text=i18n.msg["Extra Confirmation"])
...
class HOTNODE_PT_texture_save(HOTNODE_PT_parent, Panel):
    ...
    def draw(self, context):
        ...
        # Texture Save
        row = layout.row()
        row.operator("node.hot_node_texture_save", text=i18n.msg["Save Texture"])
...
def draw_ex_fast_create_preset(self, context):
    self.layout.separator()
    self.layout.prop(context.scene.hot_node_props, "fast_create_preset_name", text="", placeholder=i18n.msg["Save Nodes As"])
